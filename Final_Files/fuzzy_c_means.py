...
if __name__== '__main__':
    ...
    tuplis = []
    for i in range(membership_matrix.shape[0]):
        row = membership_matrix[i,:]
        index = np.argmax(row)
        label = df[i,44]
        tuplis.append((index,label))

    print(len(tuplis))
    # Accuracy 
    # Case 1: First Cluster is Yes and Second Cluster is No
    ct = 0
    for tup in range(len(tuplis)):
        if(tuplis[tup][0]==0 and tuplis[tup][1]=='Yes'):
            ct = ct + 1

        if(tuplis[tup][0]==1 and tuplis[tup][1]=='No'):
            ct = ct + 1

    ac1 = ct/float(df.shape[0])

    # Case 2: Second Cluster is Yes and First Cluster is No
    ct = 0
    for tup in range(len(tuplis)):
        if(tuplis[tup][0]==0 and tuplis[tup][1]=='No'):
            ct = ct + 1

        if(tuplis[tup][0]==1 and tuplis[tup][1]=='Yes'):
            ct = ct + 1

    ac2 = ct/float(df.shape[0])

    print("Maximum Accuracy: {}".format(max(ac1,ac2)))
